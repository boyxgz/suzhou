...
<html>
	...
	<body>
		...
		<div id="list-groupBuyPriceRule" class="content scaffold-list" role="main">
			<h1><g:message code="groupBuyPriceRule.list.label" args="[entityName]" /></h1>
			...
			<table>
				...
				<g:each in="${groupBuyPriceRuleInstanceList}" status="i" var="groupBuyPriceRuleInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${groupBuyPriceRuleInstance.id}">
							${groupBuyPriceRuleInstance?.plan?.product?.name} X ${groupBuyPriceRuleInstance?.plan?.product?.packingCount}， 货号为  ${groupBuyPriceRuleInstance?.plan?.product?.code}
						</g:link></td>
						
						<td>${fieldValue(bean: groupBuyPriceRuleInstance, field: "amountFrom")}</td>
