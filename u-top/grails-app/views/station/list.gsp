			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="address" title="${message(code: 'station.address.label', default: 'Address')}" />
					
						<g:sortableColumn property="dateCreated" title="${message(code: 'station.dateCreated.label', default: 'Date Created')}" />
					
						<g:sortableColumn property="enabled" title="${message(code: 'station.enabled.label', default: 'Enabled')}" />
				...
				<g:each in="${stationInstanceList}" status="i" var="stationInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${stationInstance.id}">${fieldValue(bean: stationInstance, field: "address")}</g:link></td>
					
						<td><g:formatDate date="${stationInstance.dateCreated}" /></td>
					
						<td><g:formatBoolean boolean="${stationInstance.enabled}" /></td>
