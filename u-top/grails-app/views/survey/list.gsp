			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="remark" title="${message(code: 'survey.remark.label', default: 'Remark')}" />
					
						<g:sortableColumn property="acceptComment" title="${message(code: 'survey.acceptComment.label', default: 'Accept Comment')}" />
					
						<g:sortableColumn property="dateCreated" title="${message(code: 'survey.dateCreated.label', default: 'Date Created')}" />
					
						<g:sortableColumn property="single" title="${message(code: 'survey.single.label', default: 'Single')}" />
				...
				<g:each in="${surveyInstanceList}" status="i" var="surveyInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: surveyInstance, field: "remark")}</td>
					
						<td><g:formatBoolean boolean="${surveyInstance.acceptComment}" /></td>
					
						<td><g:formatDate date="${surveyInstance.dateCreated}" /></td>
					
						<td><g:formatBoolean boolean="${surveyInstance.single}" /></td>
					</tr>
				</g:each>
				</tbody>
			</table>
