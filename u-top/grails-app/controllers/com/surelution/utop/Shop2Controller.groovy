...
//				products = PricePlanLabel.findAllByLabel(label).collect(){it.plan}
			...
			products = PricePlanLabel.findAllByLabelKey("homePage")
		}
	...
	def 'product'(Long id) {
		...
		def itemCount
		if(newOrder) {
			SaleOrderItem.findAllByOrder(newOrder).each {
				if(it.plan.id == id) {
	...
	def member() {
		def vouchers = Voucher.createCriteria().list() {
		...
		def orders = SaleOrder.createCriteria().list() {
			eq('subscriber', subscriber)
			or{
				eq('status', SaleOrderStatus.PAID)
				eq('status', SaleOrderStatus.DELIVERED)
			}
			order('dateCreated', 'desc')
		}
		
		[vouchers:vouchers, orders:orders]
	}
